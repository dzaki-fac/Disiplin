...
void main(){
  ...
  float pal = clamp(base * 0.85 + 0.62, 0.0, 1.0);
  vec3 color = mix(uColorA, uColorB, smoothstep(0.0, 0.5, pal));
  color = mix(color, uColorC, smoothstep(0.5, 1.0, pal));
...
const PixelBlast = ({
  ...
  edgeFade = 0.5,
  noiseAmount = 0
}: PixelBlastProps): React.JSX.Element => {
  const containerRef = useRef<HTMLDivElement | null>(null)
  const visibilityRef = useRef({ visible: true })
  const speedRef = useRef(speed)

  const threeRef = useRef<PixelBlastInstance | null>(null)
  ...
  useEffect(() => {
    ...
    const uniforms: PixelUniforms = {
      ...
      uTime: { value: 0 },
      uColorA: { value: getColor(0) },
      uColorB: { value: getColor(1) },
      uColorC: { value: getColor(2) },
      uClickPos: {
    ...
    if (noiseAmount > 0) {
      ...
      const noiseEffectLocal = new Effect(
        'NoiseEffect',
      ...
      noiseEffect = noiseEffectLocal
      const noisePass = new EffectPass(camera, noiseEffectLocal)
  ...
  }, [antialias, liquid, noiseAmount, colors, variant])
  ...
  useEffect(() => {
    ...
    t.uniforms.uColorB.value.set(getColor(1))
    t.uniforms.uColorC.value.set(getColor(2))
...
function resizeObserverCleanup(ro: ResizeObserver): void {
  ro.disconnect()
}
